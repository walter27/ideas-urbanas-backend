...
var clasificationSchema = new mongoose.Schema({
    ...
    image_active_route: {type: String, required: true},
...
clasificationSchema.post('save', function (clasification, next) {    
    ...
    //Actualizar Variable
    VariableModel.updateMany(   { 'obj_Clasification._id': clasification._id },
    ...
    //Actualizar Data
    DataModel.updateMany(   { 'obj_Variable.obj_Clasification._id': clasification._id },
                            { 'obj_Variable.obj_Clasification': clasification }, 
                            {"multi": true},
                                    function (err, result) {
                                        if (err) {
                                            return next(err);
    ...
    IndicatorModel.updateMany(  { 'obj_Clasification._id': clasification._id },
                                { 'obj_Clasification': clasification }, 
                                {"multi": true},
                                    function (err, result) {
                                        if (err) {
